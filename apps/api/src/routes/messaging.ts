...
const textMessageSchema = z.object({
  ...
  to: z.string().regex(/^\d{10,15}$/, { message: 'Phone number must be E.164 format digits only (10-15 chars)' }),
...
const bulkMessageSchema = z.object({
  serviceId: z.string().uuid(),
  recipients: z.array(z.string().regex(/^\d{10,15}$/)).max(500, { message: 'Maximum of 500 recipients allowed' }),
  message: z.string().max(4096),
  options: z.object({
    typingDelay: z.boolean().default(true),
...
export const messagingRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.post('/text', async (request: any, reply) => {
    ...
    if (service.status !== ServiceStatus.CONNECTED) {
      return reply.status(422).send({
        ...
        error: {
          ...
          message: 'WhatsApp instance is not connected. Please connect the service first.'
        }
      });
    }
    ...
    const { jobId, messageId } = await queueMessage(serviceId, to, message, plan, options);
  ...
  fastify.post('/bulk', async (request: any, reply) => {
    ...
    if (plan === UserPlan.FREE) {
      return reply.status(403).send({
        ...
        error: {
          code: 'FORBIDDEN',
          message: 'Bulk messaging is only available on paid tier plans (Pro/Business/Enterprise)'
        }
      });
    }
    ...
    if (service.status !== ServiceStatus.CONNECTED) {
      return reply.status(422).send({
        success: false,
        error: { code: 'INSTANCE_NOT_CONNECTED', message: 'WhatsApp instance is not connected' }
      });
    }
    ...
    for (const recipient of recipients) {
      ...
      await new Promise(resolve => setTimeout(resolve, options.interMessageDelay * 100));
    }
    ...
    return reply.status(202).send({
      ...
      data: {
        ...
        status: 'QUEUED',
        estimatedCompletion: new Date(Date.now() + recipients.length * options.interMessageDelay * 1000)
      }
    });
  });
  ...
  fastify.post('/image', async (request: any, reply) => {
    if (!request.isMultipart()) {
      return reply.status(400).send({
        success: false,
        error: { code: 'VALIDATION_ERROR', message: 'Multipart/form-data content-type expected' }
      });
    }
    ...
    if (!serviceId || !to || !fileBuffer) {
      return reply.status(400).send({
        success: false,
        error: { code: 'VALIDATION_ERROR', message: 'Missing fields: serviceId, to, and image file are required' }
      });
    }
    ...
    if (!service || service.status !== ServiceStatus.CONNECTED) {
      return reply.status(422).send({
        success: false,
        error: { code: 'INSTANCE_NOT_CONNECTED', message: 'WhatsApp instance not found or not connected' }
      });
    }

    try {
      // Direct send for media to prevent queue delays with heavy uploads
      const waMsgId = await whatsAppServiceManager.sendImageMessage(serviceId, to, fileBuffer, caption);
